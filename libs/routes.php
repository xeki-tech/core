...
class routes
{
    const separator = "-:)(:-"; // is a ridiculous separator for not generate conflicts with other tags
    ...
    public static function process_actions()
    {
        ...
        else if (isset($_POST['xeki-action'])) {
            $action_code = $_POST['xeki-action'];
            unset($_POST['xeki-action']);
            foreach (self::$actions as $action) {
                // check
                if ($action_code == $action['action']) {
                    // run handler
    ...
    private static function run_handler($handler, $vars, $module)
    {
        // is controller
        if (is_callable($handler)) {
            $handler($vars);
    ...
    private static function run_controller($handler, $vars, $module)
    {
        ...
        //modules inners urls
        if ($not_found) {
            self::$routes = array(); # reset routes
